...
class EspViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    //return position
    fun render(esp32Model: Esp32,  onClickListener: (Int) -> Unit, onclickEdit: (Int) -> Unit, onClickStatus: (Int) -> Unit) {
        ...
        if(esp32Model.nivel_agua == -1000.0)
        {
            ...
            binding.tvEspNivelAgua.text = "Nivel de agua: " + esp32Model.nivel_agua.toString() + " %"
        }
        ...
        if(esp32Model.humedad_tierra == -1000.0){
            ...
            binding.tvEspNivelHumedadTierra.text =
                "Humedad de tierra: " + esp32Model.humedad_tierra.toString() + " %"
        }
        ...
        if(esp32Model.temperatura == -1000.0 && esp32Model.humedad == -1000.0 && esp32Model.concentracion_gas == -1000.0
            && esp32Model.humedad_tierra == -1000.0 && esp32Model.nivel_agua == -1000.0 && esp32Model.last_publish == "")
        ...
        binding.botonRefrescarHolder.setOnClickListener {
            ...
            binding.botonRefrescarHolder.isEnabled = false



            val handler = android.os.Handler()
            //desactivamos 3 segundos
            handler.postDelayed({
                Log.d("EspViewHolder", "Descativado el boton 3 segundos")
                binding.botonRefrescarHolder.isEnabled = true
            }, 3000)

        }

        //Solo en el 1
        if(esp32Model.numero == 1){
            binding.ivEstadoConexion.setOnClickListener{
                onClickStatus(adapterPosition)
            }
        }
